...
function removeFirstOccurrences(str, value) {
  let strIn = str;
  if (strIn.match(value)) {
    strIn = strIn.replace(value, '');
    return strIn;
  }
  return strIn;
}
...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height - 2; i += 1) {
    middleArea.push(middleLine.join(''));
  }
  return [...topLine, ...middleArea, ...bottomLine].join('');
}
...
function encodeToRot13(str) {
  ...
  return str.split('').map((el) => {
    ...
    if (charCode < NCharCode) {
      return String.fromCharCode(charCode + 13);
    }
    if (charCode === NCharCode) {
      return 'A';
    }
    if (charCode > NCharCode && charCode <= ZCharCode) {
      return String.fromCharCode(charCode - 13);
    }
